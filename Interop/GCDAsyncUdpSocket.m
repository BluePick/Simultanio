...
- (void)notifyDidReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)context andTimeReceived:(UInt64) timeReceived
{
  LogTrace();
...
- (BOOL)enableBroadcast:(BOOL)flag error:(NSError **)errPtr
{
  ...
  dispatch_block_t block = ^{ @autoreleasepool {
    ...
    if (socket4FD != SOCKET_NULL)
    {
      ...
      int isLoopMode = 0;
      setsockopt(socket4FD, IPPROTO_IP, IP_MULTICAST_LOOP, &isLoopMode, sizeof(isLoopMode));
...
- (BOOL)enableLoopback:(BOOL)flag error:(NSError **)errPtr
{
  __block BOOL result = NO;
  __block NSError *err = nil;
  ...
  dispatch_block_t block = ^{ @autoreleasepool {
    ...
    {
      int value = flag ? 1 : 0;
      int error = setsockopt(socket4FD, IPPROTO_IP, IP_MULTICAST_LOOP,  (const void *)&value, sizeof(value));
...
- (void)doReceive
{
  UInt64 timeReceived = mach_absolute_time();
  LogTrace();
