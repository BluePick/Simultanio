...
static void _move_pthread_to_realtime_scheduling_class(const so_hptimer timer) {
  ...
  if (waitTimer) {
    ...
    uint64_t now = (1.0 * NSEC_PER_SEC * _timebase_info.denom) / _timebase_info.numer;
    computationTime = (timer->mWorkAmount * now);
  } else {
    computationTime = (timer->mWorkAmount * timer->mTimeInterval);
  }
  
  thread_time_constraint_policy_data_t policy;
  policy.period      = 0;
  policy.computation = (uint32_t) computationTime;
...
static void* _so_hptimer_wait(void* param) {
  waitTimer = true;
  so_hptimer timer = (so_hptimer) param;
  if (timer) {
    _move_pthread_to_realtime_scheduling_class(timer);
    mach_wait_until(timer->mTimeInterval);
    timer->mAction();
  }
  return NULL;
}
...
OSStatus so_hptimer_wait(so_hptimer timer) {
  if (timer) {
    timer->mNumRepetitions = timer->mMaxRepetitions;
    if (timer->mThread == NULL) {
      return pthread_create(&timer->mThread, NULL, _so_hptimer_wait, timer);
    }
    return noErr;
  }
  return -1;
